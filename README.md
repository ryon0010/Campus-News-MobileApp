...
1. What mobile design pattern(s) do you use in your current implementation of Badger News?
   ...
   Stacks - The preference elements are vertically organize  
   Lists - The news article elements are stacked vertically and supports navigation through vertical scrolling  
   Tabs - The tab at the bottom of the screen enable navigation between news and preference pages
...
2. Do you believe that Badger News (this homework) or Badger Bakery (last homework) was more mobile-friendly? Support your answer.
   ...
   I believe that Badger News is more mobile-friendly compared to Badger Bakery. Badger News offers a scrollable interface and navigation panels for switching between pages, providing a better user experience on mobile devices. In contrast, Badger Bakery appears to be confined to a single page, potentially making navigation less convenient for mobile users. 

3. What other gesture(s) could be added to Badger News? What additional functionality would these gesture(s) support?

   Pinch-to-Zoom - To enable users to zoom in or out on articles or images, providing a closer look at content.  
   Double-Tap - Double-tapping could be used for various actions, such as liking an article, saving it for later, or expanding/collapsing      sections.  
   Long-Press - Holding down on an article or image could trigger actions like sharing, bookmarking, or revealing additional options.

4. Is there opportunity to add any microinteraction(s)? If so, what would be the trigger-action pair(s)?
   
   Microinteraction: Animated Heart Icon for Likes  
   - Trigger: When a user double-taps on an article (as mentioned in the previous answer).  
   - Action: An animated heart icon appears briefly, providing visual feedback to confirm that the user has liked the article.  

   Microinteraction: Thumbnail Preview on Long-Press   
   - Trigger: When a user performs a long-press gesture on an article thumbnail.  
   - Action: A small thumbnail preview of the article's content pops up, allowing users to quickly glance at the material without navigating away.
